...
j_ = [('Software Engineer', 'SE'), ('Data Engineer', 'DE'), ('Data Analyst', 'DA'), ('Mechanical Engineer', 'ME'),
        ...
        for e in expl_lvl:
            ...
            while len(jobs_list_upper) <= total_jobs:
                ...
                l_ = (
                        l_
                        + ". Fetched "
                        + "{:3.2f}".format((len(jobs_list_upper) / total_jobs) * 100)
                        + "% jobs in this category"
                )
                l_ = (
                        l_
                        + ". Remaining "
                        + str(total_jobs - len(jobs_list_upper)).ljust(len(str(total_jobs)))
                        + " jobs"
                )
                l_ = (
                        l_
                        + ". Completed "
                        + str(len(jobs_list_upper)).ljust(len(str(total_jobs)))
                        + " jobs."
                )
                ...
                if len(jobs_list_upper) != 0:
                    ...
                    while not err:
                        try:
                            page.wait.ele_display(
                                (By.XPATH, '//*[@id="mosaic-provider-jobcards"]/ul/li[1]/div'),
                                timeout=20,
                            )
                            err = True
                        except Exception:
                ...
                with open('Final_Data/' + sample + "/" + co + "_" + e + ".tsv", "a", encoding="utf-8") as outfile:
                    for i in jobs_list:
                        b = "\t".join(i.values())
                        outfile.write(b + "\n")
                outfile.close()

                with open('Final_Data/' + sample + "/" + co + "_" + e + ".log", "a") as outfile:
                    outfile.write(l_ + "\n")
