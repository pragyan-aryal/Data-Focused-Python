...
for key in jobKeys:
    ...
    l_ = 'Currently Fetched ' + '{:3.2f}'.format((len(descriptions) / len(jobKeys)) * 100) + '% job description'
    l_ = l_ + '. Remaining ' + str(len(jobKeys) - len(descriptions)).ljust(4) + ' jobs'
...
with open(code + '_desc.tsv', 'a', encoding='utf-8') as outfile:
    for i in descriptions:
        outfile.write(i['key'] + '\t' + i['desc'] + '\t' + str(i['benefits']) + '\n')
outfile.close()
